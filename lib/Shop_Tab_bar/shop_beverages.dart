...
class _ShopBeveragesState extends State<ShopBeverages> {
  ...
  Future<List<Record>> fetchRecords() async {
    ...
    if (response.statusCode == 200) {
      ...
      // Filter the records to include only those in the "Beverages" category
      List<Record> beveragesRecords = allRecords.where((record) {
        return record.category == "Beverages";
      }).toList();

      return beveragesRecords;
    } else {
      throw Exception('Failed to load records');
    }
  }

  Future<void> addItemToOrder({required Record record}) async {
    final String url =
  ...
  Future<void> updateItemQuantity({required Record record}) async {
    final String url =
        'https://api.airtable.com/v0/appgAln53ifPLiXNu/OrderItems';
